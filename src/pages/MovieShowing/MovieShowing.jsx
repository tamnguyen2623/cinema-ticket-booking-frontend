...
const MovieShowingList = () => {
  ...
  const handleAddMovieShowing = async (values) => {
    try {
      ...
      if (modalType === "add") {
        const response = await axios.post('http://localhost:8080/movieshowing/', newMovieShowing);
        if (response.data.success) {
          fetchData();
        }
      } else if (modalType === "edit") {
        const response = await axios.put(`http://localhost:8080/movieshowing/${currentMovieShowing._id}`, newMovieShowing);
        if (response.data.success) {
          fetchData();
        }
      }
  ...
  return (
    ...
    <div className='content'>
      ...
      >
        <Form form={form} onFinish={handleAddMovieShowing} layout="vertical">
          <Form.Item name="movieId" label="Phim" rules={[{ required: true }]}>
            <Select options={movies.map(movie => ({ value: movie._id, label: movie.name }))} />
          </Form.Item>

          <Form.Item name="showtimeId" label="Suất Chiếu" rules={[{ required: true }]}>

            <Select options={showtimes.map(showtime => (
              {
                value: showtime._id, label: new Date(showtime.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
              }))} />
          </Form.Item>

          <Form.Item name="cinemaId" label="Rạp" rules={[{ required: true }]}>
            <Select options={cinemas.map(cinema => ({ value: cinema._id, label: cinema.name }))} />
          </Form.Item>
      ...
      <Table
        dataSource={movieShowings.filter((showing) =>
